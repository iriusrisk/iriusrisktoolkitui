...
def createComponents(scObject):
    ...
    for i, (name, ref) in enumerate(zip(names, refs)):
        ...
        useCasesRefs = scObject.getUseCaseIdsFromComponentId(ref)
        useCasesNames = scObject.getUseCaseNamesFromComponentId(ref)
        useCasesDescs = scObject.getUseCaseDescsFromComponentId(ref)
        usecasesLibraryClass = createUseCases(useCasesRefs, useCasesNames, useCasesDescs, ref, scObject)
...
def createUseCases(refs, names, descs, componentId, scObject):
    usecasesLibraryClass = supermod.usecasesLibraryType()
    usecaseLibraryClass = supermod.usecaseLibraryType()

    for (ref, name, desc) in zip(refs, names, descs):
        ref = str(ref)
        name = str(name)
        logger.info("Creating Use Case with Reference: " + ref)
        #We create threats for each useCase Ref
        threatRefs = scObject.getThreatIdsFromComponentIdAndUseCase(componentId, ref)
        threatNames = scObject.getThreatNamesFromComponentIdAndUseCase(componentId, ref)
        threatDescs = scObject.getThreatDescsFromComponentIdAndUseCase(componentId, ref)
...
def checkIfAllColumnsExists(excelPath):
    ...
    columnsFila2=['Id', 'Name', 'Desc', 'Id', 'Name', 'Desc', 'Id', 'Name', 'Desc', 'References', 'Id', 'Name', 'Desc', 'References', 'Id', 'Name', 'Desc', 'Test steps', 'References', 'Standards']
...
def main():
    ...
    path = Path.cwd() / "outFiles" / "spreadSheets"
